...
RUN echo "=== DEBUG: What's in /tmp/all-files? ===" && \
    ...
    echo "=== Finding ALL directories in build context (deeper search) ===" && \
    find /tmp/all-files -type d | grep -E "(public|config|src)" | sort && \
    echo "=== Finding public directory specifically ===" && \
    find /tmp/all-files -type d -name "public" && \
    echo "=== Finding config directory specifically ===" && \
    find /tmp/all-files -type d -name "config" && \
